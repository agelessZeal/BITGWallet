...
class KeyringController extends EventEmitter {
  ...
  createNewVaultAndKeychain (password) {
    console.log('createNewVaultAndKeychain in eth keyring:',password)
  ...
  submitPassword (password) {
    return this.unlockKeyrings(password)
      .then((keyrings) => {
        console.log('eth KeryingController submit password:',password,keyrings)
  ...
  addNewAccount (selectedKeyring) {
    console.log('addNewAccount:',selectedKeyring)
    ...
    return Promise.all(this.keyrings.map((keyring) => {
      return Promise.all([keyring.type, keyring.serialize()])
        ...
        // Label the output values on each serialized Keyring:
          console.log('persistAllKeyrings eth keyring:',serializedKeyringArray)
          return {
  ...
  async _restoreKeyring (serialized) {
    ...
    // console.log('restoreKeyring in eth:',serialized)
    const Keyring = this.getKeyringClassForType(type)
  ...
  async getAccounts () {
    ...
    const addrs = await Promise.all(keyrings.map((kr) => kr.getAccounts()))
      ...
      // const addresses = addrs.map(normalizeAddress)
      console.log('getAccounts in eth keyring:',addrs)
      return addrs;
