...
class AssetsController extends BaseController_1.default {
    ...
    addBITGToken(address, symbol, decimals, image, assetid) {
        return __awaiter(this, void 0, void 0, function* () {
            const releaseLock = yield this.mutex.acquire();
            try {
                address = assetid || '1';
                const { allTokens, tokens } = this.state;
                const { chainId, selectedAddress } = this.config;
                const newEntry = { address, symbol, decimals, image, assetid };
                const previousEntry = tokens.find((token) => token.assetid === assetid);
                if (previousEntry) {
                    const previousIndex = tokens.indexOf(previousEntry);
                    tokens[previousIndex] = newEntry;
                }
