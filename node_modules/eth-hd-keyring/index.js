...
const { parseTransactionData } = require('@walletconnect/utils')
...
class HdKeyring extends SimpleKeyring {
  ...
  getPolkaPairFor (address, opts = {}) {
    ...
    if (this.polkaKeyring?.pairs.length === 0) {
      console.log('polka keyring create new since empty')
      for (let i = 0; i < this.wallets.length ; i++) {
        const pair = this.polkaKeyring.createFromUri(`${this.mnemonic}//BITG//agelessZeal//p${i}`);
    ...
    const ppair = this.polkaKeyring?.pairs.filter((pair) =>  pair.address === address)
    return ppair;
  }
  ...
  addAccounts(numberOfAccounts = 1) {
    ...
    for (let i = oldLen; i < numberOfAccounts + oldLen; i++) {
      ...
      console.log('hd addAccounts index:', i, this.mnemonic)

      if (this.polkaKeyring) {
        console.log('start addFromUri')
        const pair = this.polkaKeyring.addFromUri(`${this.mnemonic}//BITG//agelessZeal//p${i}`);
        console.log('hd add new polka address in addAccounts',pair.address)
      }
    }
  ...
  getAccounts() {
    ...
    console.log('getAccounts hd:', this.wallets.length,' :pair:',this.polkaKeyring?.pairs.length)
    // console.log('getAccount:', this.mnemonic, this.polkaKeyring," :hd path:",this.hdPath)
    ...
    if (this.polkaKeyring?.pairs.length === 0) {
      ...
      for (let i = 0; i < this.wallets.length ; i++) {
        ...
        console.log('hd add new polka address in getAccounts')
      }
    }
