...
buildIosSimulatorE2E(){
	prebuild_ios
	xcodebuild -workspace ios/BITG.xcworkspace -scheme BITG -configuration Debug -sdk iphonesimulator -derivedDataPath ios/build
}
...
buildIosReleaseE2E(){
	...
	if [ "$PRE_RELEASE" = true ] ; then
		...
		xcodebuild -workspace ios/BITG.xcworkspace -scheme BITG -configuration Release -sdk iphonesimulator -derivedDataPath ios/build
	fi
}
